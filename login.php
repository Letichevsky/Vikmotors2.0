<?php
session_start();
$pass='admin88888888'; // Пароль для входа в Админку
...
if((isset($_POST['slovo'])||isset($_POST['sekret']))||($_SESSION['sekret']==md5($pass))){
    ...
    line-height: 24px;
    font-size: 31px;
    font-family: sans-serif;">Вход в панель управления</div>
			<input type="text" placeholder="Введите пароль" name="slovo" size="100" style="min-height: 60px;
    margin-bottom: 15px;
...
// Помощь
if(!isset($_GET['mode'])){
	echo('<div id="help"><p><br><h2>Вы находитесь в панели управления сайтом.</h2><p><p>
		Слева вверху находится выпадающий список, в котором Вы можете выбрать страницу, которую хотите отредактировать, выбирете её в списке а затем нажмите "Редактировать".<br/>index это главная страница, остальные внутренние, их названия вы можете увидеть в адрестной строке браузера, находясь на нужной странице.
		<p>Правее вверху находятся кнопки управления редактированием. Нажмите на "Текст" и Вы сможете редактировать текст на сайте, просто нажав по нужному абзацу или слову и заменив его на новое, после чего нажмите "Заменить текст" и изменения сохранятся.<p>
		При нажатии на кнопку "Картинки" можно менять старые картинки на новые, просто нажав по той, которую хотите заменить, загрузить на ее место новую и сохранить изменения. (Работает только в расширенной версии)
		<hr>
		...
		<p> Кнопки "HTML" и "CSS JS" дают возможность редактировать исходный код сайта, поэтому если вы не знакомы с языками HTML, CSS и JavaScript не редактируйте там ничего что-бы не поломать сайт.
		<p> Они предназначены для того что бы быстро интегрировать код метрик/аналитик/фейсбук-пикселей/онлайн консультантов и прочих полезных интеграций, требующих вставки кода.
		<br/><br/>
		<p>Если остались вопросы или нужна помощь програмиста <p>Свяжитесь со мной через Telegram: @lvlks или WhatsUp: +48507388882<br/>
		<br/>
		C уважением, Владислав.</p></div>');
};
